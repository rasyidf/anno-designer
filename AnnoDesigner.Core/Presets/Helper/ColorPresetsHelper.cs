...
namespace AnnoDesigner.Core.Presets.Helper
{
    public class ColorPresetsHelper
    {
        ...
        {
            get { return _loadedColorPresets ??= _colorPresetsLoader.Load(Path.Combine(Path.GetDirectoryName(Assembly.GetEntryAssembly().Location), CoreConstants.PresetsFiles.ColorPresetsFile)); }
        }

        private ColorScheme LoadedDefaultColorScheme
        {
            get { return _loadedDefaultColorScheme ??= _colorPresetsLoader.LoadDefaultScheme(Path.Combine(Path.GetDirectoryName(Assembly.GetEntryAssembly().Location), CoreConstants.PresetsFiles.ColorPresetsFile)); }
        }

        private BuildingPresets LoadedBuildingPresets
        {
            get { return _loadedBuildingPresets ??= _buildingPresetsLoader.Load(Path.Combine(Path.GetDirectoryName(Assembly.GetEntryAssembly().Location), CoreConstants.PresetsFiles.BuildingPresetsFile)); }
        }
