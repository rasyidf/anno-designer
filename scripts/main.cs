...
using System.Text.Json.Serialization; // Required for Source Generation attributes
using System.Threading.Tasks;

// This script converts a hardcoded C# Dictionary object (pasted by the user)
// directly into separate JSON files for each language, bypassing string parsing.

// --- FIX FOR AOT/TRIMMING COMPILATION ---
// The following context class enables System.Text.Json Source Generation.
// This resolves the runtime error about disabled reflection-based serialization.
[JsonSourceGenerationOptions(
    WriteIndented = true, // Preserve the WriteIndented = true option
    // Relaxed escaping is applied manually via the Encoder property below, 
    // as it's not a standard Source Generation option.
    GenerationMode = JsonSourceGenerationMode.Default
)]
// The type we are serializing in the foreach loop is Dictionary<string, string>.
...
{
    // A container class for the source data.
    ...
    {
        // 1. CONFIGURATION
        const string outputDirectory = "output_json"; // Directory where JSON files will be saved
        
        // Access the dictionary object directly, no string parsing needed.
        var allTranslations = SourceData.AllTranslations;

        if (allTranslations == null || allTranslations.Count == 0)
        {
            Console.WriteLine("Error: No localization data found in SourceData.AllTranslations. Please ensure the dictionary is populated correctly.");
            return;
        }

        Console.WriteLine("Starting localization data conversion from in-memory C# object...");
        
        // 3. SERIALIZATION (JSON Output)
        try
        {
            ...
            {
                // Source generation already handles WriteIndented = true (via attribute above).
                // We add the encoder here as it's a runtime option, not a static generation option.
            ...
            foreach (var kvp in allTranslations)
            {
                ...
                // Since the iteration variable is IDictionary<string, string>, 
                // we cast it to the concrete Dictionary<string, string> type 
                // to match the type used in the Source Generation context.
                var translations = (Dictionary<string, string>)kvp.Value; 
                string outputFilePath = Path.Combine(outputDirectory, $"{langCode}.json");

                // --- KEY FIX HERE ---
                // We now use the generated TypeInfoResolver instead of passing options.
                string jsonString = JsonSerializer.Serialize(translations, AppJsonContext.Default.DictionaryStringString);
                // The JsonSerializerContext handles the options (like indentation) specified in its attributes.

                await File.WriteAllTextAsync(outputFilePath, jsonString);

                Console.WriteLine($"Successfully converted {translations.Count} keys for language: {langCode}");
                Console.WriteLine($"-> JSON file written to: {outputFilePath}");
            }
        ...
        {
            // Note: The original implementation did not catch the reflection error because 
            // the error happens during the internal TypeInfo lookup when reflection is disabled.
            // This new implementation bypasses the reflection path entirely.
            Console.WriteLine($"An error occurred during file operations: {ex.Message}");
        }
    }
}